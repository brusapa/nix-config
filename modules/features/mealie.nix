    ...
    nixos =
      { config, ... }:
      {
        ...
        services.mealie = {
          ...
          settings = {
            BASE_URL = "https://recetas.${config.reverseProxy.baseDomain}";
            SMTP_HOST = "127.0.0.1";
            SMTP_PORT = 25;
            SMTP_AUTH_STRATEGY = "NONE";
            SMTP_FROM_NAME = "Recetas";
            SMTP_FROM_EMAIL = "recetas@${config.reverseProxy.baseDomain}";
            OIDC_AUTH_ENABLED = true;
            OIDC_SIGNUP_ENABLED = true;
            OAUTH_PROVIDER_NAME = "Pocket ID";
            OIDC_CONFIGURATION_URL = "https://pocketid.${config.reverseProxy.baseDomain}/.well-known/openid-configuration";
          };
        };
